...
foreach ($lista as $elem) {
	if ( ( ($_GET["megjelenes"]=="mindegy") || 
				 ($_GET["megjelenes"]!="mindegy" && $elem["megjelenes"]==$_GET["megjelenes"]) 
			 ) &&
			 ( intval($elem["ar"])>=intval($_GET["min"]) && intval($elem["ar"])<=intval($_GET["max"]) ) && 
			 ( $_GET["cim"]=="" || 
				 strtolower($_GET["cim"])==strtolower($elem["cim"]) || 
			   strpos(strtolower($elem["cim"]),strtolower($_GET["cim"]))!==false )  
		 )
